...
import { Radio, BrainCircuit, ScanLine, Gauge, ShieldCheck, type LucideIcon } from "lucide-react";
import { Particles } from "./Particles";

type Node = { icon: LucideIcon; label: string; sub: string };

const nodes: Node[] = [
  { icon: Radio, label: "KOL Signals", sub: "Curated input" },
  { icon: BrainCircuit, label: "AI Verification", sub: "Model scoring" },
  { icon: ScanLine, label: "Anomaly Detection", sub: "On-chain scan" },
  { icon: Gauge, label: "Risk Scoring", sub: "Probabilistic" },
  { icon: ShieldCheck, label: "Trader Protection", sub: "Guarded entry" },
];

export function Ecosystem() {
  return (
    <section id="ecosystem" className="relative py-32 md:py-44 bg-navy-deep text-white overflow-hidden noise-texture">
      {/* ambient gradients */}
      <div aria-hidden className="pointer-events-none absolute inset-0">
        <div
          className="absolute top-0 left-1/4 h-[600px] w-[600px] rounded-full opacity-30 animate-drift-x"
          style={{ background: "radial-gradient(closest-side, oklch(0.62 0.19 260 / 0.5), transparent)" }}
        />
        <div
          className="absolute bottom-0 right-1/4 h-[500px] w-[500px] rounded-full opacity-25"
          style={{ background: "radial-gradient(closest-side, oklch(0.7 0.13 280 / 0.5), transparent)" }}
        />
      ...
      <div className="relative mx-auto max-w-7xl px-6 lg:px-10">
        <div className="max-w-2xl">
          <p className="text-xs uppercase tracking-[0.2em] text-cyan-soft font-medium">Intelligence Network</p>
          ...
          <p className="mt-5 text-white/60 text-lg max-w-xl">
            From raw call to protected entry — every signal travels through five intelligent layers.
          </p>
        </div>
        ...
        >
          ...
          <div className="hidden md:block relative">
            {/* Connection line + animated signal pulses */}
            <svg
              className="absolute top-10 left-0 right-0 w-full h-24 -z-0"
              viewBox="0 0 1000 80"
            ...
            >
              ...
              <path
                d="M 60 40 Q 250 10, 500 40 T 940 40"
                stroke="url(#flowGrad)"
                strokeWidth="1.5"
                fill="none"
              />
              {[0, 0.6, 1.2, 1.8, 2.4].map((delay, i) => (
                <circle key={i} r="3.5" fill="oklch(0.82 0.11 220)">
                  <animateMotion
                    ...
                    path="M 60 40 Q 250 10, 500 40 T 940 40"
                  />
                  <animate
